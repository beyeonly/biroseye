...
export default function ProductScreen({ navigation, route, onCartUpdate, cartCount }) {
  ...
  const [selectedTiers, setSelectedTiers] = useState({}); // {optType: tierName}
        ...
        <View style={styles.info}>
          ...
          <View style={styles.prices}>
            <View style={styles.priceBox}>
              ...
              {/* v1.0.106: Στρογγυλοποίηση σε ακέραιο (consistency με ERP Inshop/Magazine).
                  Παλιά .toFixed(2) έδειχνε 3999.96€ αντί 4000€. */}
              <Text style={styles.price}>{totalInstallments().toFixed(0)}€</Text>
              {getInstSurcharge() > 0 && <Text style={styles.priceSub}>+{getInstSurcharge().toFixed(0)}€ επιπλ.</Text>}
            </View>
            <View style={styles.priceBox}>
              <Text style={styles.priceLabel}>Μετρητά</Text>
              <Text style={styles.price}>{totalCash().toFixed(0)}€</Text>
              {getCashSurcharge() > 0 && <Text style={styles.priceSub}>+{getCashSurcharge().toFixed(0)}€ επιπλ.</Text>}
            </View>
          </View>

          {/* Παραλλαγές / Options — v1.0.105: tier-based cascading chips */}
          {hasOptions && Object.entries(product.options).map(([optType, optData]) => {
            // v1.0.105: New tier-based structure { tiers: [{tier, products, count}], total }
            // Legacy structure was a flat array of options.
            const isTiered = optData && typeof optData === 'object' && Array.isArray(optData.tiers);
            const tiers = isTiered ? optData.tiers : [{ tier: '—', products: optData || [] }];

            // Auto-select single tier; otherwise use user choice
            const selectedTierName = selectedTiers[optType]
              || (tiers.length === 1 ? tiers[0].tier : null);
            const activeTier = tiers.find(t => t.tier === selectedTierName);
            const productsToShow = activeTier ? activeTier.products : [];

            // Hide the "—" placeholder tier from UI when it's the only one
            const showTierRow = isTiered && tiers.length > 1;

            return (
              <View key={optType} style={styles.optionGroup}>
                <Text style={styles.optionTitle}>{optType}</Text>

                {/* Tier chips row (e.g. PACK 1 / PACK 2) */}
                {showTierRow && (
                  <View style={styles.optionRow}>
                    {tiers.map(t => {
                      const isTierSelected = t.tier === selectedTierName;
                      return (
                        <TouchableOpacity
                          key={t.tier}
                          style={[styles.tierChip, isTierSelected && styles.tierChipSelected]}
                          onPress={() => {
                            resetIdleTimer();
                            // Switch tier; clear any selected variant from a different tier
                            setSelectedTiers(prev => ({ ...prev, [optType]: t.tier }));
                        ...
                        >
                          <Text style={[styles.tierChipText, isTierSelected && styles.tierChipTextSelected]}>
                            {t.tier}
                          </Text>
                          <Text style={[styles.tierChipCount, isTierSelected && styles.tierChipTextSelected]}>
                            {t.count}
                          </Text>
                        </TouchableOpacity>
                      );
                ...
                {/* Variant chips row (filtered by selected tier) */}
                <View style={styles.optionRow}>
                  {productsToShow.map((opt, i) => {
                    // For tiered: identify by variant_product_id; for legacy: by name
                    const optId = opt.variant_product_id || opt.name;
                    const selId = selectedOptions[optType]?.variant_product_id
                      || selectedOptions[optType]?.name;
                      ...
                      <TouchableOpacity
                        ...
                        onPress={() => {
                          ...
                          setSelectedOptions(prev => {
                            ...
                            // Include tier in the selection so cart sees full context
                            return {...prev, [optType]: { ...opt, _tier: activeTier?.tier }};
                          });
                        }}
                      >
                        {optPhoto && (
...
const styles = StyleSheet.create({
  ...
  tierChip: { flexDirection: 'row', alignItems: 'center', gap: 6, paddingHorizontal: 12, paddingVertical: 6, backgroundColor: '#16354f', borderRadius: 16, borderWidth: 1, borderColor: '#2d5f8a' },
  tierChipSelected: { backgroundColor: '#2ab4ea', borderColor: '#2ab4ea' },
  tierChipText: { color: '#aac4e0', fontSize: 12, fontWeight: '600', textTransform: 'uppercase' },
  tierChipCount: { color: '#7dcfed', fontSize: 11, marginLeft: 2 },
